...
public class UsuarioRepository implements IUsuarioRepository {
	...
	public List<Usuario> listar() {
		...
		List<Usuario> lstUsuario = namedJdbcTemplate.query(sql, mapeo);
	...
	public Usuario buscarUsuarioClaveEstadoPorUsuario(String documento) {
		...
		if (lstUsuario.size() == 0) {
			throw new RuntimeException("el usuario no existe -> " + documento);
		}
	...
	public List<String> buscarRolePorUsuario(String documento) {
		...
		System.out.print(lstRoles);
		if (lstRoles.size() == 0) {
			throw new RuntimeException("Usuario sin permisos -> " + documento);
		}
